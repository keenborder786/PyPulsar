...
class Bookie(Cluster):
    ...
    """
    def __init__(self , secured:bool, webservice_url:str,
                        webservice_port:str , bookie_id:str):
        """
        Parameters:
        ------------------------------
    ...
    def delete_rack_placement_bookie(self) -> Dict:
        ...
        response = requests.delete(f'{self.request_type}://{self.webservice_url}:{self.webservice_port}/admin/v2/bookies/racks-info/{self.bookie_id}')
        if response.status_code == 200:
            ...
            print(f'Request could not happen due to the following to error {response.status_code}')
        return {}
    def get_rack_placement_bookie(self) -> Dict:
        ...
        """
        response = requests.get(f'{self.request_type}://{self.webservice_url}:{self.webservice_port}/admin/v2/bookies/racks-info/{self.bookie_id}')
        if response.status_code == 200:
    ...
    def update_rack_placement_info_bookie(self , rack_info:Dict) -> Dict:
        ...
        """
        response = requests.post(f'{self.request_type}://{self.webservice_url}:{self.webservice_port}/admin/v2/bookies/racks-info/{self.bookie_id}' , headers = {'Content-Type': 'application/json'}
